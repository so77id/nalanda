...
8. La entrega por correo la hace #273 (ADR-0072): una publicación por control, un correo por alumno emparejado, desde la cuenta del profesor. Queda en V2 el nombrado por ALUMNO del PDF adjunto.
...
next to RUTs; step 7 grew email delivery in #273; lo que le queda a G es Canvas grade posting y el CSV.
...
| D | **#271 · #272 · #273 ✅** | [#271](https://github.com/so77id/nalanda/issues/271) · épica [#270](https://github.com/so77id/nalanda/issues/270) | **Curso y roster, importación desde Canvas.** #271 entregó las tablas (`course`, `student`, `enrollment`, `user_secrets`), el cliente de Canvas, el token cifrado del profesor y las pantallas `/profile` y `/courses`. [#272](https://github.com/so77id/nalanda/issues/272) ✅ cerró el cruce `reading.rut_read` ↔ `student.rut`: `control.course_id` + `reading.student_id` (migración `00015`), el dominio `internal/domain/matching`, el emparejamiento en cada lectura y en cada edición manual de RUT, y las cuatro pantallas (curso, control con nombres, alumno, matriz). **NO hubo migración de relleno**: la reasociación retroactiva es un botón re-ejecutable (`POST /courses/{id}/rematch`, `POST /admin/rematch`) porque una migración corre una vez y queda inútil la próxima vez que cambie la lista — y porque la imagen de producción es `FROM scratch` y no tiene shell (ADR-0034 §Consequences). Política completa: ADR-0071. [#273](https://github.com/so77id/nalanda/issues/273) ✅ **cerró la épica**: el profesor aprieta *Publicar* y cada alumno emparejado recibe su PDF corregido por correo, enviado **desde la cuenta de Gmail del propio profesor** (OAuth, scope `gmail.send`, una segunda autorización separada del login). Cuatro modos de despacho elegidos al arranque por `NALANDA_EMAIL_MODE`, cuya opción por defecto es `stub` a propósito; `POST /controls/{id}/publish`, `/test-send` y `/unpublish`; una quinta `jobs.Kind` que no toca al worker de AMC. [#287](https://github.com/so77id/nalanda/issues/287) ✅ **corrigió lo que se vio al usarlo con un curso real el 2026-09-08**: la publicación pasó a registrarse **por copia** (`reading.published_at` / `published_grade`, migración `00019`), así que *Publicar* es **reanudable** y apretarlo dos veces no le manda a nadie una segunda copia; se fue el 409, se fue `/unpublish` — reemplazado por `/resend-all`, que borra los sellos y no envía nada — y llegaron el envío por alumno (`POST /controls/{id}/copies/{n}/publish`, síncrono, desde la página de revisión), la columna *Envío* con las cuatro situaciones por copia y su motivo, y el `23/25 enviadas` del listado. También arregló un defecto de #273 que llegó a los estudiantes: el correo componía `FormatGrade(NumericGrade(...))` y re-escalaba la nota real como si fuera un puntaje crudo sobre el número de preguntas: **demasiado alta** en un control corto (saturando en 7,0) y **demasiado baja** en uno largo — un 7,0 real llegaba como 5,2 en uno de diez preguntas. Política completa: ADR-0073. **No se usó un API transaccional** (Resend y compañía exigen dominio verificado, y UDP no verifica udp.cl): política completa en ADR-0072 | C1, y un token de Canvas |
| G | V2 | [Discussion #163](https://github.com/so77id/nalanda/discussions/163) | **Publish GRADES** — CSV export and Canvas grade posting. **La entrega por correo del PDF corregido ya no está aquí: la cerró #273** (ADR-0072). Queda además **one annotated PDF per student**, invoking `apps/amc-worker`'s `/annotate` with the real roster D provides (files nombrados por alumno, no por `copy-N`). The copy-numbered annotated PDF shipped in #190 (ADR-0048) without a roster; what remains for G is the per-STUDENT naming; la entrega la hace #273 nombrando el adjunto `correccion-copia-N.pdf`. Runs retroactively over V1 readings the day D lands, because `readings.rut_read` + `rut_override` already carry the identity `/annotate` needs. Moved to V2 (2026-08-17): V1 grades live in the database and on the web table; if Miguel needs to move them somewhere else before V2, a query against the SQLite file gets them out. Per-student annotated PDF confirmed as V2 (2026-08-17, refinement conversation) — /annotate needs a roster, WP-D delivers it, doing it in V1 would be a synthesised roster hack | D, F |
