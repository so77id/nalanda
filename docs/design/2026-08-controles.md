...
| C15 | **Hosting is deferred; the first test bed is DocumentBuddy's Jetson.** Development runs locally under docker-compose. WP-C2 (#150) made something to deploy — the professor login — and #162 put it on DocumentBuddy's Jetson Nano behind Tailscale Funnel on port 8443 (DocumentBuddy's ADR-010, ADR-014). That is a **test bed, not a home**: the choice between staying there permanently and moving to a VPS is still open, and ADR-0037 records the measurements the permanent decision will need — image size on arm64, running-container footprint, and cost. See also ADR-0037's re-deferrals of the five WP-C2 review triggers that fire at the first deploy. | 2026-08-15, revised 2026-08-17 |
...
that is now false for the middle two. `apps/server` runs on DocumentBuddy's
Jetson Nano behind Tailscale Funnel on port 8443, with a daily S3 backup and a
Telegram health monitor of its own. There is still no VPS and no automated
deploy pipeline — ADR-0037 records the shape and the reason the Jetson is a
test bed rather than a permanent home, and the permanent choice is still open.
