...
| L8 Manual              | Human visual/functional verification. Three procedures no agent can run: `apps/amc-worker/PAPER-CHECK.md` (print, mark, scan), `apps/server/GOOGLE-CHECK.md` (a real Google login) and `apps/server/CANVAS-CHECK.md` (a real Canvas instance, #271)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | PR checklist                                                                    | Pre-PR                                    |
...
**Both protocols run inside the builder image the `Dockerfile` pins, never
on a host toolchain** (2026-09-05). The image is
`golang:1.25-alpine@sha256:3eb6c2b3db8d55e38537302edb510b4417f8a115efbd5906d131ceba9468e29a`,
so the suite runs on the same `go1.25.13` that builds the shipped binary and
that `go.mod` declares. A laptop Homebrew Go tests a standard library
production never runs — measured on 2026-09-05, the host was `go1.26.6`
against a module declaring `1.25.13`, which is the same toolchain gap
§govulncheck below exists to catch, and CI does not cover it either
(`server.yml` reads `go-version-file: apps/server/go.mod`).

Two things the naive translation gets wrong, both measured:

- **Mount the repository ROOT and set the workdir — never `apps/server`
  alone.** `internal/domain/controls/tex` reads
  `../../../../../amc-worker/tests/fixtures/control-demo.tex`, so a
  narrower mount fails `TestFixtureAndGeneratorAgreeOnTheLoadBearingRules`
  with `no such file or directory` rather than with anything that names the
  mount.
- **`-race` needs a C toolchain the alpine image does not carry.** It fails
  with `go: -race requires cgo; enable cgo by setting CGO_ENABLED=1`, and
  the image ships no gcc. The pre-PR step is therefore its own invocation:
  add `-e CGO_ENABLED=1` and prefix with `apk add --no-cache build-base &&`.
  Never drop `-race` to make the container line shorter — see the `-race`
  note below for what it is there to find.

The two named volumes are the module and build caches; without them every
run re-downloads the module graph.

**Per-commit** (in the pinned image, from the REPO ROOT):

```bash
docker run --rm -v "$PWD":/src -w /src/apps/server \
  -v nalanda-gomod:/go/pkg/mod -v nalanda-gocache:/root/.cache/go-build \
  golang:1.25-alpine@sha256:3eb6c2b3db8d55e38537302edb510b4417f8a115efbd5906d131ceba9468e29a \
  sh -c 'test -z "$(gofmt -l .)" && go vet ./... && go build ./... && go test ./...'
```

The commands that container runs, and why each is what it is (the `gofmt`
caveat especially), are unchanged:
...
**Pre-PR** — same image, from the repo root, with `-race` needing its own
line for the cgo reason above:

```bash
docker run --rm -e CGO_ENABLED=1 -v "$PWD":/src -w /src/apps/server \
  -v nalanda-gomod:/go/pkg/mod -v nalanda-gocache:/root/.cache/go-build \
  golang:1.25-alpine@sha256:3eb6c2b3db8d55e38537302edb510b4417f8a115efbd5906d131ceba9468e29a \
  sh -c 'apk add --no-cache build-base && go test -race -count=1 ./...'
```

`govulncheck` runs in the same image (it needs network, which the default
bridge gives it); `docker build` and the compose tail stay host `docker`
commands. The battery itself, unchanged:
...
`oidctest.Provider` against an `httptest` JWKS, which verifies the verifier and
says nothing about whether a real OAuth client, a real redirect URI and a real
account produce a session. The procedure is `apps/server/GOOGLE-CHECK.md` and it
is **required whenever the login path changes** — the OIDC adapter, the callback,
the cookie, or `NALANDA_PUBLIC_URL`. Same rule and same reason as
`apps/amc-worker/PAPER-CHECK.md`.
...
- **A grammar is invisible to the suite, and fails silently by design.**
  ...
  yet their only effect is colour, jsdom mocks CodeMirror away, and a failed
  grammar load is swallowed on purpose (a missing grammar costs colour, and the
  thing the reader came to RUN reports its own failures). Adding a language, or
  touching the grammar path, is verified in a browser against the built site: a
  fence in that language is actually coloured. #122 shipped the split with the
  whole consumer side untested and 989 cases green.
- **A promise read with `use()` during render must be memoised, and what that
  costs depends on what is above it.** `loadCatalog()` in `catalog/registry.ts` is
  the worked case (#122): it is called inside the component body, so an
  unmemoised promise is a NEW promise on every render and the component re-suspends
  forever. Two mutations of the same line, at two commits, gave two different
  answers — and the difference is the lesson. With a `<Suspense>` boundary above
  it, only the *second* reader in a session misbehaved: three cases reddened, all
  of them navigations between two pages that read the resource, and no mount case
  moved. With the boundary removed (which is what ships), the first mount never
  commits and the failure is loud: **33 cases across two files**, mount and
  navigation alike. So: memoise it, and do not conclude from a green mount suite
  that the memo is pinned — check what boundary, if any, is catching the
  suspension, because that is what decides which cases can see the defect.
