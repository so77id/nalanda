...
| Verify the Google login by hand | [`../../apps/server/GOOGLE-CHECK.md`](../../apps/server/GOOGLE-CHECK.md) | One of the three checks no agent can run: a real OAuth client, a real sign-in, a second account refused, a sign-out that sticks. Required whenever the login path changes, because no test in this repository reaches Google |
| Verify the Canvas integration by hand | [`../../apps/server/CANVAS-CHECK.md`](../../apps/server/CANVAS-CHECK.md) | The third check no agent can run: a real token against the real UDP instance, the courses and roster queries, where the RUT lives (`user.sisId`), the token round trip on `/profile`, and the sweep proving the token reaches neither a log line nor a database column. Required whenever the Canvas path changes (#271, ADR-0068/0069) |
| Verify the control engine on paper | [`../../apps/amc-worker/PAPER-CHECK.md`](../../apps/amc-worker/PAPER-CHECK.md) | One of the three checks no agent can run: print, mark, scan, read, compare. Required before trusting a synthetic green run |
