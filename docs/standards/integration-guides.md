...
| Add a database migration | [`backend-code-style.md` §Adding a migration](backend-code-style.md#adding-a-migration) | Changing `apps/server`'s schema: file naming, the embed root, why the Down block is documentation rather than a tested path, and why a version number is never reused even after its file is deleted |
