...
| Course content style | [`guides/course-content-style.md`](guides/course-content-style.md) | The voice of the course — formal first-person plural, neutral Spanish, no cross-references between sections or slides, noun-phrase titles. Applies to every `.mdx` under `content/courses/` |
| Teach a data structure | [`guides/teach-a-data-structure.md`](guides/teach-a-data-structure.md) | The shape of a class in the Estructuras de Datos unit: the act shape, the four repeated formats (TDA contract, invariant list, cost table with its invariant column, cabo suelto), the figure-panel rule, and when a widget is decided. Worked case: `17-edd-introduccion.mdx` (#277) |
...
| Add a database migration | [`backend-code-style.md` §Adding a migration](backend-code-style.md#adding-a-migration) | Changing `apps/server`'s schema: file naming, the embed root, why the Down block is documentation rather than a tested path, and why a version number is never reused even after its file is deleted; and **which side of the foreign keys the table is on** — rebuilding a PARENT of `ON DELETE CASCADE` children deletes every child row, and `PRAGMA foreign_keys=OFF` cannot save you because it is a no-op inside goose's transaction (#287) |
...
| Verify the Google login by hand | [`../../apps/server/GOOGLE-CHECK.md`](../../apps/server/GOOGLE-CHECK.md) | One of the checks no agent can run: a real OAuth client, a real sign-in, a second account refused, a sign-out that sticks. Required whenever the login path changes, because no test in this repository reaches Google |
| Verify the Canvas integration by hand | [`../../apps/server/CANVAS-CHECK.md`](../../apps/server/CANVAS-CHECK.md) | One of the checks no agent can run: a real token against the real UDP instance, the courses and roster queries, where the RUT lives (`user.sisId`), the token round trip on `/profile`, and the sweep proving the token reaches neither a log line nor a database column. Required whenever the Canvas path changes (#271, ADR-0068/0069). §6b (#272) adds the JOIN: that the eight digits AMC reads off a real sheet equal the eight digits `sisId` yields for the same person — no test can reach it, since the suite fakes Canvas on one side and fixtures the readings on the other |
| Verify the publication path by hand | [`../../apps/server/GMAIL-CHECK.md`](../../apps/server/GMAIL-CHECK.md) | The check no agent can run for the mail path: the Google Cloud Console setup, a consent that grants `gmail.send` AND one that withholds it, a dry run whose log masks recipients, a real send whose PDF opens, the mode gate, and §6 — which exists to MEASURE the refresh-token lifetime ADR-0072 could not settle from documentation, and to write the answer back into it. Required whenever the Gmail adapter, the `/profile` connect flow, `internal/infra/email/`, the message builder or `NALANDA_EMAIL_MODE` changes (#273, ADR-0072) |
| Verify the control engine on paper | [`../../apps/amc-worker/PAPER-CHECK.md`](../../apps/amc-worker/PAPER-CHECK.md) | One of the checks no agent can run: print, mark, scan, read, compare. Required before trusting a synthetic green run |
| Deploy `apps/server` + `apps/amc-worker` to the Jetson | [`../../infra/local/DEPLOY-JETSON.md`](../../infra/local/DEPLOY-JETSON.md) | Bringing both apps up on DocumentBuddy's Jetson (#162, #175, ADR-0038): the off-box prerequisites (AWS bucket + IAM via `infra/deploy/jetson/provision-jetson-iam.sh`, Telegram bot, Google OAuth second redirect URI), Tailscale Funnel on port 8443, the `.env` shape, `docker compose pull` (~1 GB for amc-worker the first time), verification (`/health`, `GOOGLE-CHECK.md` §7), backup test-restore, the two proxy-trust measurements (benign AND adversarial) before flipping `NALANDA_TRUST_PROXY_HEADERS=true`, and rollback. L8 for AMC generation/reading goes through [`../../apps/amc-worker/PAPER-CHECK.md`](../../apps/amc-worker/PAPER-CHECK.md). |
