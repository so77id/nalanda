...
**Amended by:** #287 (2026-09-08) — the FIRST synchronous caller that writes rows an async job also writes. `handler.Controls.PublishCopy` sends one copy's correction on the request goroutine, under its own 25 s deadline, and stamps `reading.published_at` — which the `publish` job also stamps. The single-goroutine ordering does NOT cover it: the runner serialises jobs against each other, never against a request. The substitute is a check-then-act refusal (`handler.publishJobInFlight`, reading `LatestForControlByKind` + `Status.IsTerminal()`) plus the per-copy stamp itself; the residual window — a job starting between the check and the send — is accepted at the cost of one duplicate message, rather than paid for with a lock across two goroutines. Full reasoning in ADR-0073 §5. **A new synchronous route that touches rows a `jobs.Kind` touches inherits this rule** (`add-a-backend-endpoint.md`, the fourth synchronous rule).
**Amended by:** #298 (2026-09-22) — a DONE job may carry one short message for the professor. `jobs.Notice` is the mirror image of `jobs.Failure`: a handler returns it through the same error slot, the runner records the row as `done` with the sentence in `job.notice` (migration 00021), and the banner renders it under "lista". Worked case: an analyse that re-read copies already mailed, or met pages it could not place. The `Handler` signature is unchanged on purpose — no handler with nothing to say changes shape. The same issue adds two named exceptions to this ADR's rules, argued in ADR-0075 §5: `handler.ScansReset` is the ONE synchronous `amcworker.Client` call (it only removes files and never waits on the client lock — `TryLock`, `ErrAnalyzerBusy`; a second such call needs both properties and an ADR), and it departs from the #287 clause above that a synchronous route touching a Kind's rows inherits the flash-and-fail-open rule: it answers 409 and fails CLOSED on a jobs-store read error, because a wipe racing this control's own analyse cannot be undone.
