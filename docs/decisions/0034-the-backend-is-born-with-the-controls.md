...
**Amended by:** #150 (2026-08-16) — the empty `00001_init.sql` was deleted and the auth schema numbered `00002`; the image measured 12.2 MB after the port · #166 (2026-08-17) — WP-E landed and the server now shares the `amc-work` named volume with `apps/amc-worker`; the two-container seeding-order rule is recorded in §Consequences · #249 (2026-08-27) — the "row is committed AND files are on disk, or neither" atomicity of `Service.Create` and `Service.UploadScan` is now bounded to their SYNC halves (`Service.PrepareControl`, `Service.SaveUploadedBatch`); the async halves (`Service.GenerateAssets`, `Service.AnalyzeBatch`) deliberately preserve the row + files on worker refusal so the professor does not lose the pool snapshot to a transient outage. The full runner design lives in ADR-0050 · #261 (2026-08-27) — deleting a control is now a two-step archive → purge, with soft-delete on `deleted_at` (migration `00013`) and a typed-name confirmation on the destructive step; the ON DELETE CASCADEs this ADR §Consequences left on `control_pregunta`, `copia`, `reading`, `answer`, `annotated_copy` and `job` are what makes the purge one statement. Full three-gate design in ADR-0052. · #271 (2026-09-05) — the roster, the Canvas client and the sealed professor token · **#287 (2026-09-08) — the THIRD atomicity shape (#273's "commit the row before the irreversible external effect") is REVERSED, and the reversal is what makes it a shape rather than a rule.** The record moved from the batch to the COPY (`reading.published_at` / `published_grade`, migration `00019`; `control.published_sent` dropped in `00020`), so the unit of irreversible work is one message and the row is committed immediately AFTER it. Stamping ahead only ever bought crash-safety for a batch that could not be resumed; a per-item record makes the run resumable and the trade disappears. The generalisation: **commit after the external effect when the effect can be split small enough to repeat safely, and before it when it cannot.** Full design in ADR-0073.
**Source:** #149 (WP-C1), design `docs/design/2026-08-controles.md` §C10, §C11 · #273 (2026-09-07) — publication adds `published_at`, `publication_mode` and `published_sent` on `control` and `gmail_address` on `users`, and a THIRD atomicity shape: the row is committed BEFORE an irreversible EXTERNAL effect, so a crash costs the un-sent half rather than a duplicate mailing. Every earlier rule here concerns rows and files on one box, both of which this server can undo; an email cannot be recalled. Full design in ADR-0072 §5.
