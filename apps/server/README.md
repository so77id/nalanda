...
| `POST /controls/{id}/publish` | Stamps the control published and enqueues the sending job (issue #273, ADR-0072). Form field `mode` is `real` (to the students) or `staging` (to the professor's own address). Refused unless the correction is closed, the control has a course, and the acting professor has connected a Gmail account — each with its own message, because each has a different repair. A second publish answers **409**: the request is well-formed and the resource is already in the state it asks for, and publication is one-way in v1 |
| `POST /controls/{id}/test-send` | The rehearsal: the whole batch to one typed address, changing nothing (issue #273). Same gates MINUS the already-published one — rehearsing a control that has already gone out is exactly what a professor does when a student says nothing arrived. The address is parsed with `net/mail.ParseAddress` and only the bare address is kept, so a `Nombre <a@b>` form does not put a display name in a To header nobody wrote |
