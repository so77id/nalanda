...
| `POST /controls/{id}/publish` | Stamps the control published and enqueues the sending job (issue #273, ADR-0072). Form field `mode` is `real` (to the students) or `staging` (to the professor's own address). Refused unless the correction is closed, the control has a course, the acting professor has connected a Gmail account, AND this server is configured to actually send (`NALANDA_EMAIL_MODE`) — each with its own message, because each has a different repair; a non-delivering transport is a 422 naming the variable and the control is NOT stamped. A second publish answers **409**: the request is well-formed and the resource is already in the state it asks for, and publication is one-way unless the professor explicitly deshace it, which the 409 message points them at |
| `POST /controls/{id}/test-send` | The rehearsal: the whole batch to one typed address, changing nothing (issue #273). Same gates MINUS the already-published one and MINUS the delivers gate — rehearsing on a server that delivers nothing is coherent and changes no state, though the flash says so — rehearsing a control that has already gone out is exactly what a professor does when a student says nothing arrived. The address is parsed with `net/mail.ParseAddress` and only the bare address is kept, so a `Nombre <a@b>` form does not put a display name in a To header nobody wrote |
| `POST /controls/{id}/unpublish` | Undoes a publication so the control can be published again (issue #273 review, ADR-0072 §5). It unsends NOTHING, and every string on the path says so. It exists because publication was one-way with no exceptions, so any run that stamped without reaching anybody — every send failing, a transport that delivers nothing, a `staging` choice used as a rehearsal — left the class permanently unreachable through the app. The confirmation reports `control.published_sent`, so the professor decides with the number in front of them whether the people who already have their correction may receive it twice |
