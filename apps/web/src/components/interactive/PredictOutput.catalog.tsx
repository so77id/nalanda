...
export const predictOutputCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    'When the point of a passage is that the reader is going to guess wrong. Traps like `new String("hola") == new String("hola")` and the Integer cache at 128 teach far more when the JVM is the one saying so than when the author asserts an answer in prose. Same shape as the general principle in ADR-0019 §7 — a written verdict is feedback, not evidence. §7\'s two named forgeries are student-side; this component defeats the author-side twin, an answer asserted in prose that no gate can falsify. ' +
