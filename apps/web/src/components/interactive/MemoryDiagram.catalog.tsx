...
export const memoryDiagramCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    ...
    'The compiler is fetched only when the reader presses the button. Mounting one costs 33.95 kB raw / 13.13 kB gzip across four lazy chunks (measured on the built site, #122): the component, the runtime seam and the Java module. It used to cost a CodeMirror grammar on top — 16.14 kB gzip for a highlighter this component never draws, since it renders its own listing — and #122 moved grammars behind loadGrammar(id) so it no longer does. Still four requests and a JVM per instance, so a page of them is a page of JVMs.',
  props: [
    {
      name: 'title',
      type: 'string',
