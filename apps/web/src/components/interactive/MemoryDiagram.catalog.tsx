...
export const memoryDiagramCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    ...
    'The compiler is fetched only when the reader presses the button. Mounting one costs 33.95 kB raw / 13.12 kB gzip across four lazy chunks (measured on the built site, #122): the component, the shared runtime-loading hook, the runtime seam and the Java module. It used to cost a CodeMirror grammar on top — a highlighter this component never draws, since it renders its own listing — and #122 moved grammars behind loadGrammar(id) so it no longer does. Still four requests and a JVM per instance, so a page of them is a page of JVMs. ' +
    'A `trace` fence declaring any of the three reserved platform class names at top level is refused as an authoring error before any JVM boots; naming one in a comment or a string is fine. A NESTED class called `NalandaTrace` is accepted by that guard and is still wrong here: the tracer calls are injected into the author’s own class, where a member type of that name captures them and the diagram draws whatever it printed (#123).',
  props: [
    {
      name: 'title',
      type: 'string',
