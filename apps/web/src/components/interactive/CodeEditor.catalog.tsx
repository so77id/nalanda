...
export const codeEditorCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    ...
    'Three Java class names are reserved by the platform, `NalandaLauncher`, `NalandaCheck` and `NalandaTrace`: a program declaring one of them at TOP LEVEL is refused before it compiles — entry class or not, `class`, `interface` or `enum` alike (#123) — so never use them in an example. A nested declaration is fine, and naming one in a comment or a string is fine. Java output is capped at 48KB, after which one `[nalanda] salida truncada` line appears — that bounds how far the console grows, and does NOT make a print-heavy program finish (ADR-0020 §6: 60 000 println had still not returned after 300s).',
  props: [
    {
      name: 'language',
