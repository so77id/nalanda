...
export const codeEditorCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    ...
    'Three Java class names are reserved by the platform, `NalandaLauncher`, `NalandaCheck` and `NalandaTrace`: a program whose MAIN class is one of them is refused before it compiles, so never use them in an example (a secondary declaration is not caught — see docs/security-notes.md). Java output is capped at 48KB, after which one `[nalanda] salida truncada` line appears — that bounds how far the console grows, and does NOT make a print-heavy program finish (ADR-0020 §6: 60 000 println had still not returned after 300s).',
  props: [
    {
      name: 'language',
