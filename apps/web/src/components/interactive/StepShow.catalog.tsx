...
export const stepShowCatalogEntry: CatalogEntry = {
  ...
  family: 'interactive',
  description:
    'Code above, arbitrary JSX below — stacked, both full width, both walked in sync by prev / next / reset controls. A generic primitive: the visual can be a memory diagram, a call stack, a tree, a queue, a sequence figure — anything the author writes inside a <Step>.',
  whenToUse:
    'Whenever the lesson requires the reader to see a fragment of code and its effect together, one step at a time. Stacked rather than side by side: two columns inside a document measure scaled the drawing to unreadable (#116). In presentation it breaks out of the Slide prose column to HALF the viewport on its own, so never wrap it in <PresentationWide> (ADR-0067), one step at a time. Introduced in #209 as the reversal of ADR-0028: the memory picture used to be drawn from an execution trace (a Java compile + JVM per diagram); <StepShow> takes author-written state and pairs it with a highlighted listing, at a fraction of the bundle. ' +
    'For memory specifically, use <MemoryVisual state={…}> inside each <Step> — it consumes a { frames, objects } state where references are values on the variables/fields themselves ({ kind: "ref", id }), never a top-level list (ADR-0049 §Decision-2). For other visuals (call stacks, trees, hash tables, sequence diagrams), drop any JSX inside a <Step> instead. ' +
