...
export const stepShowCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    'Whenever the lesson requires the reader to see a fragment of code and its effect side by side, one step at a time. Introduced in #209 as the reversal of ADR-0028: <MemoryDiagram> used to draw from an execution trace, which needed a Java compile and a JVM per diagram; <StepShow> takes author-written state and pairs it with a highlighted listing, at a fraction of the bundle. ' +
    'For memory specifically, use <MemoryVisual state={…}> inside each <Step> — it consumes the same {frames, objects, refs} shape the trace machinery used to produce. For other visuals (call stacks, trees, hash tables, sequence diagrams), drop any JSX inside a <Step> instead. ' +
