...
export const exerciseCatalogEntry: CatalogEntry = {
  ...
  whenToUse:
    ...
    'The class names `NalandaLauncher`, `NalandaCheck` and `NalandaTrace` are reserved: a student program declaring one of them at TOP LEVEL is refused before it compiles, entry class or not (#123), with a message saying so. A nested declaration is fine — it compiles to `Solucion$NalandaLauncher.class` and overwrites nothing. The rule reaches the `test` fence too, where the failure is reported to the document author rather than blamed on the student.',
  props: [
    {
      name: 'title',
      type: 'string',
